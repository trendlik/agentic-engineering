...
The execution-context fields let the reference class be filtered/compared by model: `platform` names the adapter (e.g. `claude-code`, `antigravity`), `coordinator_model` is the coordinator's own concrete model ID (self-reported), and each `*_model` field is the alias the corresponding phase's sub-agent was spawned with — see "Model assignments & Capability Tiers" below for which phase maps to which tier/alias. A phase whose agent never ran that run (no review issues → no `fix_model`, a green first CI run → no `ci_fix_model`) stays `null`, never defaulted to the tier's assigned model. These are a claim by the coordinator about what it ran, not something the script itself can verify.

`scripts/backfill-outcomes.sh` can seed the ledger from issues that were already implemented and merged/closed before the ledger existed, reconstructing what it can from `gh issue view` / `gh pr view` history and leaving the rest — the plan/friction signals (`plan_file_count`, `clarify_rounds`, `plan_revisions`, `review_loops`) and the execution-context fields (`platform` and every `*_model`), none of which `gh` history records — null. See `backfill-outcomes.sh`'s own header comment for the authoritative field list, rather than restating it here.
