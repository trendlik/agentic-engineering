...
Fetches issue #42, clarifies requirements with the user, drafts and gets approval on an implementation plan, then runs sequential sub-agents (implement → test → review) in an isolated git worktree. Loops on review findings until clean, opens a PR, then watches CI and spawns fix agents until all checks pass. Ends with a retrospective that diagnoses bottlenecks and routes findings: project-specific learnings into the target repo's `.implement-issue/LEARNINGS.md` (requiring prior explicit user approval before any file writes or commits occur) and outcome ledger entries into `.implement-issue/outcomes.jsonl`, with an option to commit them directly to the feature PR branch as part of the feature PR after CI passes, or commit separately.
...
| 8 | Retrospective | Main agent → diagnose bottlenecks → route findings & record outcomes; option to commit to feature PR branch |
...
Phase 8 appends (or upserts) one line per run, best-effort and non-blocking, via `scripts/record-outcome.sh`. At a glance, each line carries: `issue`, `title`, `pr`, `labels`, `outcome` (merged/closed/aborted), size signals (`plan_file_count`, `files_changed`, `diff_loc`, `commits`), friction signals (`clarify_rounds`, `plan_revisions`, `review_loops`, `ci_fixes`), `wall_clock_hours`, execution context (`platform`, `coordinator_model`, `implement_model`, `test_model`, `review_model`, `fix_model`, `ci_fix_model`), and provenance (`skill_sha`, `skill_version`, `recorded_at`). Fields that can't be reconstructed for a given run are stored as JSON `null` — never guessed or defaulted to 0.

The execution-context fields let the reference class be filtered/compared by model: `platform` names the adapter (e.g. `claude-code`, `antigravity`), `coordinator_model` is the coordinator's own concrete model ID (self-reported), and each `*_model` field is the model tier alias the corresponding phase's sub-agent was spawned with (e.g. `sonnet` on Claude Code, `gemini-3.5-flash`/`gemini-3.5-pro` on Antigravity — record the tier's assigned model alias, NOT the sub-agent tool `TypeName` such as `self` or `research`) — see "Model assignments & Capability Tiers" below for which phase maps to which tier/alias. A phase whose agent never ran that run (no review issues → no `fix_model`, a green first CI run → no `ci_fix_model`) stays `null`, never defaulted to the tier's assigned model. These are a claim by the coordinator about what it ran, not something the script itself can verify.

`scripts/backfill-outcomes.sh` can seed the ledger from issues that were already implemented and merged/closed before the ledger existed, reconstructing what it can from `gh issue view` / `gh pr view` history and leaving the rest — the plan/friction signals and the execution-context fields, none of which `gh` history records — null. See `backfill-outcomes.sh`'s own header comment for the authoritative field list, rather than restating it here.
...
- Emit each role-boundary checkpoint question **exactly once**, as the single turn-ending message, and only **after** that boundary's `state.sh`/artifact calls have returned. Do not pre-announce or narrate the question before running those tool calls, and do not add your own separate "recorded successfully" sentence on top of it — the scripted prompt (e.g. "Plan approved and recorded…") already confirms the recording. On adapters that echo a task/tool-call description back to the user (e.g. Antigravity's `ManageTask`), the question text goes only in the final message, never also in that task/tool-call description — that echo is exactly how a checkpoint prompt ends up shown twice, once as preamble and once as the real message. A checkpoint prompt that appears twice, in any form, is a bug (chattier models and echo-prone adapters are the most prone to it).
- **Classify every checkpoint reply before acting on it**: exactly one of (a) explicit affirmative approval → proceed, (b) explicit stop/handoff → stop, or (c) anything else — a question, a comment, an ambiguous or partial reply → not consent. For (c), answer or address the reply, then re-ask the same checkpoint and wait; never treat it as a go-ahead. Concretely: a user asking "why do I get this twice?" in reply to a checkpoint is a question about the checkpoint, not approval to proceed — answering it does not by itself authorize starting the next phase. See WORKFLOW.md's **Checkpoint discipline** section for the full rule and its rationale.
- Same rule at every other role boundary: never auto-continue from clarification into planning, or from implementation into testing, or from testing into review — always ask, and apply the same reply-classification rule at each. This also covers the Phase 6 minor-issue and architecture-waiver decision points, which are checkpoints in the same sense even though they don't gate a phase transition. In worktree mode, push the branch before any of these checkpoints (except Clarify→Plan, which has no branch yet) — Phase 0's resume logic checks the remote, not local worktree state, so unpushed commits aren't recoverable by a resuming session. In local mode the Phase 3/4 and Phase 6-fix checkpoints deliberately invert this: the work stays uncommitted and unpushed *for review*, and the Local-mode review gate commits + pushes on approval (or on an explicit stop, which is a handoff) — see WORKFLOW.md's "Execution modes". Full definition of both rules: WORKFLOW.md's **Checkpoint discipline** section.
...
- Phase 8: always runs after Phase 7 (pass or stop-and-report); tally loop counts, diagnose root causes, then route each proposal by scope: project-scoped → `.implement-issue/LEARNINGS.md` in the target repo (requires prior explicit user approval before any file writes or commits occur — stopping to wait for user response), skill-scoped → evidence-backed issue on the skill repo. Retrospective changes (`outcomes.jsonl` and approved `LEARNINGS.md` entries) can be committed directly to the feature PR branch (upon user choice) after CI passes so they form part of the same PR, or committed separately. Direct edits to the skill directory are reserved for maintainers with push access who explicitly choose them (see WORKFLOW.md Phase 8 Step 4).
